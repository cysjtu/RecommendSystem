...
import librec.ext.CachedSlopOne;
...
import librec.rating.CachedItemKNN;
import librec.rating.CachedUserKNN;
...
import librec.rating.TopNUserKNN;
...
public class LibRec {
	...
	public SparseMatrix rateMatrix, timeMatrix;
	...
	public void execute(String[] args) throws Exception {
		...
		// collect results
		String filename = (configFiles.size() > 1 ? "multiAlgorithms" : Recommender.algoName) + "@" + Dates.now() + ".txt";
	...
	public void printEvalInfo(Recommender algo, Map<Measure, Double> ms) throws Exception {
		...
		String time = String.format("traintime:'%s',testtime:'%s'", Dates.parse(ms.get(Measure.TrainTime).longValue()),
	 ...
	 *            email attachment
	 */
	public void notifyMe(String attachment) throws Exception {
	...
	public Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
		...
		case "topnuserknn":
			return new TopNUserKNN(trainMatrix, testMatrix, fold);
		case "cacheduserknn":
			return new CachedUserKNN(trainMatrix, testMatrix, fold);
			
			
		case "itemknn":
			return new ItemKNN(trainMatrix, testMatrix, fold);
		case "topnitemknn":
			return new TopNItemKNN(trainMatrix, testMatrix, fold);
		case "cacheditemknn":
			return new CachedItemKNN(trainMatrix, testMatrix, fold);
		...
		case "cachedslopeone":
			return new CachedSlopOne(trainMatrix, testMatrix, fold);
	...
	public void writeData(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
